...
## Preparation completion counts — September 20, 2026

Unified LeetCode calendar, overview, activity and Stage counts around the same qualifying AC events. A same-problem repeat at least three hours after the last counted event adds one; midnight and Stage boundaries do not reset the interval. The cumulative metric includes a separately tracked trusted profile-history floor, displays ≥ when incomplete, and identifies undated or out-of-range history without assigning invented dates. Behavioral now counts the first saved nonempty answer without a completion button; edits and clearing preserve its existing preparation date. Old saved answers and explicit events share one deduplicated projection, including event tombstones.

Local full-shell fixtures used synthetic data, never the user's account. On desktop 1280px and mobile 390px, 71 distinct LeetCode problems plus 14 qualifying repeats displayed 85 completions, Stage values 34/51/0 and 255 activity points for the day with 51 events. Saving a second Behavioral answer changed its cumulative count from 1 to 2 and today's score from 10 to 20; editing it again kept the count at 2. The removed completion button was absent, mobile had no horizontal overflow, and final browser warning/error logs were empty. Fixture files and synthetic account data are ignored and not shipped.

All 275 related Node tests, two personal API round-trip tests, strict production build, growth and route checks passed. Regression coverage includes cooldown boundaries, cross-midnight/Stage events, shared explicit time zones, incomplete trusted history, old-answer backfill, clear/rewrite, cloud merge and deleted-event handling. Independent review found no remaining P1/P2 issues. The live account was on the login page, so these results verify the calculation fix rather than reconcile the user's exact 80-plus history.

## LeetCode history and Stage count semantics — September 20, 2026

Independent code and local-data review of the history import, client projection, counting model and Stage display found no remaining P1/P2 issues. This entry records local checks only; it does not certify browser appearance or deployment.

- **新完成** counts different problems whose first accepted submission in complete known history belongs to the Stage. **总完成** includes qualifying repeats at least three hours after the last counted AC. All Stage ranges remain start-exclusive and end-inclusive in the selected local time zone.
- A complete private export establishes a dated cutoff, not trusted public-source coverage. Closed days before that cutoff remain verifiable after subsequent refreshes. Later unknown intervals stay partial; missing values render as dashes. Partial Stage totals are labeled as recorded, without a ≥ claim, because earlier missing ACs can change their Stage attribution. Confirmed first-solve subsets and cumulative completion lower bounds retain ≥.
- Extension and file imports preserve the five coverage fields and capture timestamp through `prepareHistory`; extra payload fields are not forwarded. The backend binds imported records to the current profile and link, keeps public sync records separate, deduplicates overlapping submission IDs with public records taking precedence, and rejects conflicting identities/timestamps. Imports do not increase Guardian progress or replace public profile statistics.
- Independently executed: 22 client, counting and rendered-markup tests; 10 collector-extension checks; and 8 backend import/owner-isolation/Guardian tests. All passed. The cutoff regression now returns unknown for an uncovered later Stage, preserves zero on fully covered empty days, and does not turn historical closed Stages partial solely because of a later refresh.
- Read-only processing of the private repair file through the actual frontend sanitizer and backend importer preserved 85 solved problems and 184 accepted submissions, including the complete-export cutoff. Reimport was idempotent. Independent local calculation produced 85 first solves, 104 qualifying completions and 19 repeats; Stage pairs were 50/64, 35/40 and 0/0. The first two ranges were ready; the current range and cumulative total remained partial after the capture instant. No account identifier, credential, source file or individual history record is included here or added to the repository.
