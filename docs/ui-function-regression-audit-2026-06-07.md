...
| `336-ops-alert-production-fixture-summary.json` | Ops alert production fixture：隔离生产 env 正例通过，short/placeholder webhook token、placeholder/local/private/raw-IP/incomplete signoff 反例被拒，本地 webhook `--smoke` 投递和 HMAC 签名通过且输出不暴露 token 或完整 dashboard URL。 |
