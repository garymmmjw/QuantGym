...
- 2026-06-21 media storage CDN Range smoke：为真实 bucket/CDN 签收补充 public Range GET 覆盖。`scripts/check-media-storage-config.mjs --live` 现在在 public media URL 完整 GET 后发送 `Range: bytes=0-15`，要求返回 `206`、匹配的 `Content-Range` 和正确字节切片；fake CDN fixture 支持 206 响应，`npm run check:media-storage:production-fixture`、`npm run build:media-storage-packet`、`npm run check:external-launch-blockers` 与 `npm run check:ui-contracts` 均记录 `liveFixturePublicRangePass`，用于防止视频/大媒体播放时因 CDN 不支持 Range 造成卡顿或无法 seek。
...
- 2026-06-21 jobs source raw-IP URL 加硬：发现 production source URL 在拒绝私网后仍可能接受公网裸 IP。`scripts/check-jobs-source-config.mjs` 现在要求 production source URL 使用 HTTPS DNS hostname，`npm run check:jobs-source:production-fixture` 新增 public IP source URL 反例，`npm run build:jobs-feed:publication-packet` 和 `npm run check:external-launch-blockers` 也记录 raw-IP source URL guidance。
...
- 2026-06-21 Chrome Web Store published-signoff raw-IP URL 加硬：发现 Chrome 发布签收会通过非 store host 规则间接拒绝 raw-IP listing/evidence URL，但没有独立证明发布证据必须使用 DNS hostname。`scripts/check-chrome-store-publication.mjs` 现在在 HTTPS URL helper 中显式拒绝 raw IP host，`npm run check:chrome-store-publication:fixture` 新增 raw-IP listing/evidence URL 反例，packet 与 external blocker 汇总也记录这项覆盖。
...
- 2026-06-21 Postgres cutover evidence URL raw-IP 加硬：发现最终切库签收已明确拒绝 raw-IP target host，但 evidence URL 只拒绝 private-network，仍可能接受公网 raw IP。`scripts/check-postgres-cutover.py` 现在要求 evidence URL 使用 HTTPS DNS hostname，`npm run check:postgres-cutover:export-smoke` 新增 public-IP evidence URL 反例，packet、UI contract 和 external blocker 汇总也记录 `evidenceUrlRawIpRejected`。
...
- 2026-06-21 ops alert webhook signature 加硬：API runtime webhook 和 `check:ops-alerts:production -- --smoke` 现在都会用 `QUANTGYM_ALERT_WEBHOOK_TOKEN` 对原始 compact JSON body 生成 `X-QuantGym-Alert-Signature: sha256=<hmac>`。`npm run check:ops-alerts:runtime-smoke` 验证 7 条真实 alert 签名，`npm run check:ops-alerts:production-fixture` 验证 production smoke 签名，`npm run build:ops-alert-edge-packet` 的 webhook contract 要求接收器用常量时间比较校验签名；`npm run check:external-launch-blockers` 也记录 `localWebhookSmokeSignatureValid` 和 `packetIncludesWebhookSignatureContract`。
...
- 2026-06-18 deployed production-boundary verifier 加硬：发现本地 `npm run build` 会把 `config.js`/`dist/config.js` 写成本地 `127.0.0.1` 端点，之后直接跑 `npm run verify:production-boundaries` 容易把“线上边界检查”误打到本地服务。`scripts/verify-production-boundaries.mjs --deployed` 现在直接读取 `https://beta.quantgym.app/config.js` 的线上 API/LLM 端点，并在输出中记录 `scope`、`endpointSource`、`cloudApiEndpoint` 和 `llmEndpoint`；部署模式也优先使用线上 Google Client ID，避免本地 `.env` 覆盖导致 token audience 误判。`npm run verify:production-boundaries:deployed:paste-token` 会隐藏读取新鲜 Google ID token，Google provider login 成功后自动把返回的 QuantGym cloud session 用于 deployed LLM resume/PDF 检查，并在 LLM 两项通过后刷新 `333-production-boundaries-deployed-services-summary.json`。`npm run check:ui-contracts` 允许该 deployed 证据保持 4 pass/1 skip 的 Google-token-only partial，或在提供新鲜 token 后升级为 5 pass/0 skip 的完整 pass。
- 2026-06-21 static build provenance：`scripts/build-static-site.mjs` 现在把 Git build commit/branch/source 写入 `dist/config.js`，并生成匹配的 `dist/version.json`。新增 `npm run check:static-build-config` 自动构建、校验 runtime config 不含 OpenAI key、校验 `version.json` 与 `config.js` build metadata 一致，并刷新 `321-static-build-config-summary.json`；release-readiness 总闸门已接入该检查，便于后续用线上 `/version.json` 判断静态站是否部署到目标 commit。
...
- 2026-06-21 Deployed Google token helper 修正：`npm run google:token-helper:deployed` 仍写入被 Git 忽略的 `artifacts/google-id-token-helper.html`，但 deployed 模式现在输出外部浏览器 Console handoff，引导用户在 `https://beta.quantgym.app` 上运行 Google Sign-In snippet，而不是在 `127.0.0.1` 用线上 OAuth Client ID 登录。`npm run check:google-token-helper` 覆盖 local 直接按钮和 deployed Console snippet 两条路径，确认 deployed helper 警告 `origin_mismatch`、不在 loopback 加载 GIS script、不把 token 写入 artifact，并指向 `npm run verify:production-boundaries:deployed:paste-token`。证据见 `356-google-token-helper-flow-summary.json`。
...
| `321-static-build-config-summary.json` | 静态 build config 复验：`dist/config.js` 包含本地公开 endpoint、Google Client ID、Git build provenance，不包含 OpenAI key；`dist/version.json` 与 `dist/config.js` build metadata 一致；strict mode 拒绝本地非 HTTPS endpoint。 |
| `322-ui-contract-gate-summary.json` | UI contract gate 复验：`npm run check:ui-contracts` 覆盖 21 个 React route、3 个 shell 合约、37 个关键 JSON 证据文件和 92 个非空截图证据文件。 |
...
| `356-google-token-helper-flow-summary.json` | Google token helper flow gate：local helper 保持 127.0.0.1 直接 Google 按钮；deployed helper 改为 `https://beta.quantgym.app` Console snippet，避免线上 OAuth Client ID 在 loopback 触发 `origin_mismatch`。 |
| `326-browser-evidence-manifest-summary.json` | Browser evidence manifest：审查和 smoke 文档中的 280 个 browser-audit 证据引用全部存在，229 张图非空，51 个 JSON 合法。 |
...
| `331-postgres-cutover-export-smoke-summary.json` | Postgres cutover export smoke：临时 API/SQLite 生成真实 fixture，验证 `/api/health` 暴露运行时数据库 backend/writable/foreignKeys，验证默认导出脱敏、full export 保留迁移输入，redacted export 与 row-limited/truncated include-sensitive export 都不能通过 cutover checker，并验证 complete cutover signoff 正例及错误签收反例，包括 private/raw-IP/malformed target host、unsafe database、private/raw-IP/credential/query evidence URL，以及非 Postgres runtime backend 和 raw-IP/query runtime health URL。 |
...
| `334-ops-alert-runtime-smoke-summary.json` | Ops alert runtime smoke：临时 webhook/API 验证 404、两次 auth 401 和 auth 429 rate-limit 四条告警均送达、Bearer token 和 HMAC 签名正确、payload 全量脱敏。 |
...
| `336-ops-alert-production-fixture-summary.json` | Ops alert production fixture：隔离生产 env 正例通过，short/placeholder webhook token、placeholder/local/private/raw-IP/incomplete signoff 反例被拒，本地 webhook `--smoke` 投递、HMAC 签名和 receiver verification ack 通过，无 verification ack 的 receiver 被拒，且输出不暴露 token 或完整 dashboard URL。 |
| `337-media-storage-production-fixture-summary.json` | Media storage production fixture：隔离生产 env 正例通过，local/HTTP/private/raw-IP/placeholder/raw public base/URL credential/weak credential/unsafe bucket 或 prefix 等反例被拒，fake S3/CDN `--live` 正例、public Content-Type、public Range GET 和 public GET 失败清理路径通过。 |
| `338-jobs-source-production-fixture-summary.json` | Jobs source production fixture：隔离生产 env 正例通过，source URL/token/cache/timeout/max-bytes/catalog 反例被拒，包括 private/raw-IP/embedded credentials/query URL 和 short token；fake live feed 正例和包含 invalid/future postedAt 的坏 feed 反例通过。 |
| `339-chrome-store-publication-fixture-summary.json` | Chrome store publication fixture：本地发布 handoff 正例通过，published-signoff fixture 绑定 Chrome item id、detail listing URL、published 状态、manifest version 和 upload SHA-256，且 malformed/HTTP/raw-IP/non-store/non-detail/mismatched/placeholder/credential/query 反例均被拒；真实 Chrome Web Store 发布仍需 `npm run check:chrome-store-publication:published` 带开发者账号产生的 listing 证据通过。 |
...
| `355-apex-www-domain-summary.json` | Apex/WWW domain smoke：实时诊断 `quantgym.app`、`www.quantgym.app`、`beta.quantgym.app` 的 DNS/HTTPS 状态；当前 beta 入口健康，裸域和 WWW 经 `quantgym-apex-redirect` Worker 302 到 beta 后返回 200，`npm run check:apex-www-domain -- --require-clear` 是域名清场复验命令。 |
| `357-render-llm-deploy-status-summary.json` | Render LLM deploy status：复验 root 和 `llm-proxy` 两种 Render clean install 都可 import `undici`，所有支持的 start command 均通过 `/health`，线上 API/LLM health 与 `beta.quantgym.app/version.json` 匹配当前 commit，并记录 Gmail 审查中当前 commit/最近几小时无新 Render 失败邮件。 |
| `341-external-launch-blockers-summary.json` | External launch blockers：统一跟踪 5 个剩余正式外部 blocker、已清掉的 apex/WWW、ops alert、jobs feed、Render build-filter 项和 1 个 browser journey 持续扩展项；默认 tracking pass 但 `launchReadiness` 保持 `blocked`，`--require-clear` 会在 blocker 未清空时失败；release-readiness 内嵌运行时会跳过旧 `323` 自引用并保持独立 `341` summary 不被覆盖。 |
