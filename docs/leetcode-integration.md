...
- Successful syncs also fill missing problem difficulties from LeetCode's public question metadata, including older saved problems outside the recent feed. This bounded, cached lookup is best effort and does not alter completion times or counts. Until metadata is available, the difficulty badge shows a dash instead of a completion status.
