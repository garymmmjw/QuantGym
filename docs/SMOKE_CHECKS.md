...
token, valid HMAC signature, status sequence `404,401,401,429`, ISO timestamps,
and no request bodies, credentials, auth tokens, synced state, community payloads, or problem payloads;
...
signoff inputs, keeps raw tokens/full dashboard URLs out of output, verifies the
production webhook-smoke signature contract, rejects receivers that do not
...
- [x] Ops alert runtime smoke: `npm run check:ops-alerts:runtime-smoke`
  ...
  response, then verifies all four webhook payloads are delivered, signed, and
  sanitized.
- [x] Ops alert production fixture: `npm run check:ops-alerts:production-fixture`
  ...
  action, verifies webhook-smoke HMAC signing plus receiver verification ack,
  rejects generic 2xx receivers without that ack, and verifies invalid
  production smoke attempts do not deliver webhooks.
...
- [x] Postgres cutover export smoke:
  ...
  prefixes, target row count, app-DB-active confirmation, observed postgres
  runtime backend, runtime health URL host, backup confirmation,
  ...
  or query strings, non-postgres runtime backend evidence, raw-IP/query-bearing
  runtime health URLs, SHA mismatches, row-count mismatches, inactive app database
