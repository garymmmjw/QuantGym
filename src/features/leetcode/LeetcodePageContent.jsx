...
function LeetcodeWorkspace({ lc }) {
  ...
  return <main className="lc-page" aria-labelledby="lc-title">
    ...
    {lc.error && <div className="lc-feedback is-error" role="alert">{leetcodeError(lc.error, en)} <button type="button" className="lc-text-button" disabled={lc.busy} onClick={() => lc.reload?.()}>{t("重试", "Retry")}</button></div>}
