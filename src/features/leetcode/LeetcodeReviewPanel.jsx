...
export function LeetcodeReviewPanel({ lc, practiceSessions = [], pool, allProblems, selectedProblem, onSelect, now, onLockChange }) {
  ...
  return <section className="lc-review lc-memory-review" aria-labelledby="lc-review-title">
    <div className="lc-review-main">
      ...
      </div> : <div className="lc-review-start"><span className="lc-review-symbol" aria-hidden="true">↻</span><p>{nextDue ? t(`当前筛选有 ${pool.length} 道题。先从最早到期的开始。`, `${pool.length} problems match your filter. Start with the earliest due.`) : pool.length ? t("当前筛选没有到期或待首次复习的题，可以休息一下，也可以随机温习。", "No due or first-review problems match this filter. Take a break, or draw a problem for extra practice.") : t("还没有符合筛选条件的已通过题目。", "No solved problems match this filter.")}</p><div className="lc-actions"><button className="lc-button is-primary" type="button" disabled={!nextDue || lc.busy} onClick={() => select(nextDue)}>{t("开始复习", "Start review")} ↗</button><button className="lc-text-button" type="button" disabled={!pool.length || lc.busy} onClick={() => select(drawReviewProblem(pool, "", Math.random, { now, submissions: lc.data.submissions, practiceSessions, connection }))}>{t("随机抽一道", "Draw randomly")}</button></div></div>}
      <p className="lc-review-footnote">{t("随机抽取会偏向更久没做、练习次数更少的题，依据通过记录、复习反馈和已完成的 Coding OA。打开或抽取题目不会记作复习。建议时间按本地时区显示。", "Random draws favor problems practiced longer ago and fewer times, using accepted submissions, saved review feedback, and completed Coding OA attempts. Opening or drawing a problem does not record a review. Suggested times use your local timezone.")}</p>
    </div>
