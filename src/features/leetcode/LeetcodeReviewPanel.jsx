...
export function LeetcodeReviewPanel({ lc, practiceSessions = [], pool, allProblems, selectedProblem, onSelect, now, onLockChange }) {
  ...
  return <section className="lc-review lc-memory-review" aria-labelledby="lc-review-title">
    <div className="lc-review-main">
      ...
      <p className="lc-review-footnote">{t("随机抽取会偏向更久没做、练习次数更少的题，依据通过记录、复习反馈和历史编程复盘。打开或抽取题目不会记作复习。建议时间按本地时区显示。", "Random draws favor problems practiced longer ago and fewer times, using accepted submissions, saved review feedback, and past coding reviews. Opening or drawing a problem does not record a review. Suggested times use your local timezone.")}</p>
    </div>
