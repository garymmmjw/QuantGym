...
export function LeetCodeConnection({ connectionState, compact = false }) {
  ...
  return <section className={`lc-connection${compact ? " is-compact" : ""}`} aria-labelledby="lc-connect-title">
    ...
    {lc.error?.status === 401 ? <p className="lc-muted">{t("请先点击上方「重新登录」恢复 QuantGym 连接，然后继续关联力扣。", "Use Sign in again above to reconnect QuantGym, then link LeetCode.")}</p> : !lc.enabled ? <p className="lc-muted">{t("当前记录保存在此设备。请在账户页启用云同步，再关联力扣。", "Your records are saved on this device. Enable cloud sync in Account to connect LeetCode.")}{!compact && <> <Link to="/account?section=data">{t("前往账户 →", "Go to Account →")}</Link></>}</p> : <>
      {connection && !editing ? <>
