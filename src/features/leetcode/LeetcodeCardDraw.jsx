...
export function LeetcodeCardDraw({ pool, selectedProblem, lc, practiceSessions, now, busy, headingRef, onSelect, onDrawingChange, autoDraw = false }) {
  ...
  const problem = autoDraw ? revealed : selectedProblem;
  ...
  const disabled = autoDraw || busy || Boolean(draw);
  ...
  function choose(problem, index = Math.floor(hand.length / 2)) {
    ...
    const finish = () => {
      ...
      setDraw(null);
      if (!autoDraw) {
        setHand(drawReviewCards(latest.current.pool, { ...latest.current.options, previousSlug: problem.slug }));
        setDeal(value => value + 1);
      }
    };
    setHovered(null);
    ...
    const sourceRect = cards.current[index]?.getBoundingClientRect();
    const sceneRect = scene.current?.getBoundingClientRect();
    const targetRect = target.current?.getBoundingClientRect();
    const flight = sourceRect && sceneRect && targetRect ? {
      "--flight-x": `${sourceRect.left + sourceRect.width / 2 - sceneRect.left - 40}px`,
      "--flight-y": `${sourceRect.top + sourceRect.height / 2 - sceneRect.top - 56}px`,
      "--flight-dx": `${targetRect.left + targetRect.width / 2 - sourceRect.left - sourceRect.width / 2}px`,
      "--flight-dy": `${targetRect.top + targetRect.height / 2 - sourceRect.top - sourceRect.height / 2}px`,
    } : {};
    animating.current = true;
    onDrawingChange(true);
    setDraw({ phase: "drawing", index, problem, flight });
    // The owner saves this one revealed card; drawing never writes completion data.
    timers.current.push(window.setTimeout(() => {
      setRevealed(problem);
      latest.current.onSelect(problem);
      setDraw({ phase: "revealing", index, problem, flight });
    }, 780));
    timers.current.push(window.setTimeout(finish, 1270));
  }
  ...
  return <div className={`lc-card-draw${autoDraw ? " is-automatic" : ""}${draw ? ` is-${draw.phase}` : ""}${!hand.length ? " is-empty" : ""}`} ref={scene} aria-busy={Boolean(draw)}>
      ...
      <div className="lc-card-fan" role="group" aria-label={autoDraw ? t("Quanty 的抽卡牌组", "Quanty's card deck") : t("选择小鲨鱼手中的一张卡", "Pick a card from Quanty's hand")}
        ...
        {hand.map((item, index) => {
          ...
          return <button key={`${deal}-${item.slug}`} ref={element => { cards.current[index] = element; }} type="button"
            className={`lc-hand-card${hovered === index ? " is-hovered" : ""}${draw?.index === index ? " is-picked" : ""}`}
    ...
    <div className="lc-card-draw-content">
      <div className="lc-reveal-slot" ref={target}>
        ...
        </article> : <div className="lc-draw-invitation"><span className="lc-invitation-icon" aria-hidden="true"><Shuffle size={21} /></span><h3>{hand.length ? autoDraw ? t("Quanty 正在抽卡", "Quanty is drawing your card") : t("下一题，由你来抽", "Your next problem awaits") : t("暂无可抽取的题目", "No matching problems")}</h3>{!autoDraw && <p>{hand.length ? t("从 Quanty 手里挑一张，或点击抽取。", "Pick from Quanty's hand, or let Quanty draw for you.") : t("调整下方筛选后再试。", "Adjust your filters to try again.")}</p>}</div>}
      </div>
      {!autoDraw && <div className="lc-draw-controls"><button ref={controls} type="button" className="lc-button is-primary lc-draw-trigger" disabled={disabled || !pool.length} onClick={drawRandom}><Shuffle size={15} aria-hidden="true" />{draw ? t("抽取中…", "Drawing…") : problem ? t("再抽一题", "Draw again") : t("抽取题目", "Draw a problem")}<ArrowRight size={15} aria-hidden="true" /></button></div>}
      <span className="lc-draw-announcement lc-sr-only" role="status" aria-live="polite">{draw?.phase === "drawing" ? t("Quanty 正在抽卡", "Quanty is drawing a card") : problem ? `${t("抽到了", "Selected")} ${problem.frontendId || ""} ${title}` : ""}</span>
    </div>
    {draw?.phase === "drawing" && <div className="lc-flying-card" style={draw.flight} aria-hidden="true"><CardBack /></div>}
  </div>;
}
