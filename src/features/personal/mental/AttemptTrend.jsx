...
export const AttemptTrend = memo(function AttemptTrend({ trials, currentSettings, language, selectedId, disabled, onSelect, dayKey }) {
  ...
  function exportCsv() {
    ...
    anchor.href = url;
    anchor.download = `mental-math-attempts-${new Date().toISOString().slice(0, 10)}.csv`;
  ...
  return <section className="pm-analysis" aria-label={en ? 'Attempt progress' : 'Attempt 成绩趋势'}>
    ...
    <p className="pm-note pm-filter-description">{settingsFilter === 'all' ? (en ? 'Different durations and number ranges affect scores. Use matching settings to compare your progress.' : '不同训练时长和数字范围会影响得分；比较进步时建议选择相同设置。') : formatAttemptSettings(settingsFilter === 'current' ? currentSettings : history.settingsOptions.find(option => option.key === settingsFilter)?.settings, language)}</p>
