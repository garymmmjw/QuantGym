  ...
  return <section className="personal-mental pm-reasoning" aria-label={TRAINER_LABELS[trainer]}>
    ...
    {!active && <form ref={setupRef} className="pm-setup" onSubmit={start}>
      ...
      <div className="pm-setup-toolbar pm-reasoning-settings">
        ...
        <label>{en ? 'Difficulty' : '难度'}<select aria-label={en ? 'Difficulty' : '难度'} value={draft.difficulty} onChange={event => { const value = event.target.value; setDraft(previous => ({ ...previous, difficulty: value })); }}>{['easy', 'medium', 'hard'].map(level => <option key={level} value={level}>{difficultyLabel(level, language)}</option>)}</select></label>
        {trainer === 'sequence' && <label>{en ? 'Sequence type' : '序列类型'}<select aria-label={en ? 'Sequence type' : '序列类型'} value={draft.sequenceType} onChange={event => { const value = event.target.value; setDraft(previous => ({ ...previous, sequenceType: value })); }}>{['numbers', 'letters', 'mixed'].map(type => <option key={type} value={type}>{sequenceTypeLabel(type, language)}</option>)}</select></label>}
      </div>
      <div className="pm-ready">
        <span className="pm-ready-label">{en ? 'YOUR NEXT SESSION' : '下一场训练'}</span><p className="pm-ready-clock">{clock(effectiveSettings.durationSeconds * 1000)}</p>
        ...
        <button type="submit" className="pm-primary" data-pm-start disabled={Boolean(state.activeTrial)}>{en ? 'Start practice' : '开始训练'} <span aria-hidden="true">↗</span></button>
        <p className="pm-note">{en ? 'Enter fullscreen · 5 seconds to get ready. One submission per question. Review the rule, then continue; the timer keeps running.' : '全屏进入 · 5 秒准备。每题提交一次，看解析后继续；期间仍计时，结束自动返回。'}</p>
      </div>
    </form>}
    {active && <PracticeFocus active language={language} title={TRAINER_LABELS[trainer]} onExit={exitPractice}>
    ...
    {active && !preparing && <div className="pm-arena">
      ...
      {question && <form className="pm-reasoning-question" onSubmit={event => { event.preventDefault(); act({ type: 'submit' }); }}>
        ...
        <div className="pm-answer-controls"><button type="button" className="pm-text-button" onClick={() => act({ type: 'skip' })}>{en ? 'Skip question' : '跳过本题'}</button><span>{en ? 'One submission per question' : '每题一次提交'}</span></div>
      </form>}
      {feedback && <div className="pm-reasoning-feedback">
        ...
        <div className="pm-answer-controls"><button ref={nextRef} type="button" className="pm-primary" onClick={() => act({ type: 'next' })}>{en ? 'Next question' : '下一题'} →</button></div>
      </div>}
    </div>}
    </PracticeFocus>}
