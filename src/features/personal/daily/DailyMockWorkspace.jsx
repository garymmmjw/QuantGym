    ...
    {session && !showSetup && <>
      ...
      <div className="pd-workspace-grid">
        ...
        <div className="pd-content">
          {selected === 'mental' && <section className="pd-mental-panel"><div className="pd-section-heading"><div><span className="pd-eyebrow">MENTAL MATH</span><h2>{t('先找到你的节奏', 'Find your rhythm')}</h2><p>{t(`本轮 ${session.settings.mentalSeconds} 秒。计时结束后，本板块自动完成。`, `This round lasts ${session.settings.mentalSeconds} seconds. The section completes when its timer ends.`)}</p></div>{session.mentalCompletedAt && <span className="pd-complete-label">✓ {t('已完成', 'Complete')}</span>}</div>{session.mentalCompletedAt ? <div className="pd-mental-finished"><strong>{t('这一轮心算已记录。', 'This mental math round is recorded.')}</strong><p>{t('完整逐题用时和散点图可以在 Mental math 页面查看。', 'See per-question timing and scatter plots on the Mental math page.')}</p>{!progress.complete && <button type="button" className="pd-primary" onClick={nextIncomplete}>{t('继续下一项', 'Continue to the next item')} →</button>}</div> : <MentalMathTrainer key={session.id} state={state} update={update} language={language} dailySessionId={session.id} durationSeconds={session.settings.mentalSeconds} onComplete={onMentalComplete} />}</section>}
          {question && <section className="pd-question-panel" aria-labelledby="pd-question-title">
            ...
            <div className="pd-prompt">{en ? question.promptEn || question.prompt : question.prompt}</div>
