...
export function BehavioralWorkspace({ state, update, language }) {
  ...
  return <div className="behavioral-prep">
    <header className="bp-heading">
      ...
      <p className="bp-progress"><strong>{started} <span>/ {BEHAVIORAL_PREP_QUESTIONS.length}</span></strong>{t('题已写入回答', 'answers written')}</p>
    </header>
    ...
    <div className="bp-layout">
      <nav className="bp-question-list" aria-label={t('Behavioral 问题列表', 'Behavioral questions')}>
        {[
          ...
          ><span className="bp-question-number">{String(index + 1).padStart(2, '0')}</span><span>{item.label}</span><span className={`bp-draft-dot${hasPersonalBehavioralAnswer(state, item) ? ' has-draft' : ''}`} aria-label={hasPersonalBehavioralAnswer(state, item) ? t('已写入回答', 'Answer written') : t('待准备', 'To prepare')} /></button>)}
        </section>)}
