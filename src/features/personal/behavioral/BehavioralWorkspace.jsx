...
export function BehavioralWorkspace({ state, update, language }) {
  ...
  const started = BEHAVIORAL_PREP_QUESTIONS.filter(item => getBehavioralAnswer(item, answers).trim()).length;
    ...
    <div className="bp-layout">
      <nav className="bp-question-list" aria-label={t('Behavioral 问题列表', 'Behavioral questions')}>
        {[
          ...
          ><span className="bp-question-number">{String(index + 1).padStart(2, '0')}</span><span>{item.label}</span><span className={`bp-draft-dot${getBehavioralAnswer(item, answers).trim() ? ' has-draft' : ''}`} aria-label={getBehavioralAnswer(item, answers).trim() ? t('已有草稿', 'Draft started') : t('待准备', 'To prepare')} /></button>)}
        </section>)}
      ...
      <article className="bp-editor" key={question.id} aria-labelledby="bp-question-title">
        ...
        <div className="bp-answer-actions"><span id="bp-answer-stats">{words} {t('词', 'words')}{words > 0 && ` · ≈ ${Math.round(words / 135 * 60)} sec`}</span><button type="button" disabled={!answer.trim()} onClick={copyAnswer}>{t('复制回答', 'Copy answer')}</button></div>
