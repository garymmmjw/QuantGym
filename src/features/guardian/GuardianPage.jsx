          ...
          <div className="guardian-workspace">
            <div className="guardian-primary-column">
              <section className="guardian-panel guardian-records" aria-labelledby="guardian-records-title">
                ...
                {questions.length ? <div className="guardian-question-table"><table><thead><tr><th scope="col">练习题目</th><th scope="col">类型</th><th scope="col">次数</th><th scope="col">最近完成</th></tr></thead><tbody>{questions.map((question, index) => <tr key={`${question.source || question.kind}-${question.id}-${index}`}><td><strong>{question.title || question.titleEn || "练习题目"}</strong>{question.titleEn && question.title && question.titleEn !== question.title && <small>{question.titleEn}</small>}{question.source === "manual" && <span className="guardian-source-label">手动记录</span>}{question.source === "legacy" && <span className="guardian-source-label">历史汇总</span>}</td><td><span className="guardian-question-kind">{KIND_LABELS[question.kind] || question.kind || "练习"}</span></td><td>{Number(question.count || 1)}</td><td><time dateTime={question.completedAt}>{displayTime(question.completedAt, timeZone)}</time></td></tr>)}</tbody></table></div> : <div className="guardian-empty"><span className="guardian-empty-icon"><GuardianIcon name="book" size={26} /></span><h3>这一天还没有练习记录</h3><p>学习者完成刷题并同步后，记录会显示在这里。</p></div>}
