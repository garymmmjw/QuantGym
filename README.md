...
准备做邀请制内测时，先看 [BETA_LAUNCH.md](./BETA_LAUNCH.md)。网页部署参数集中在 [config.js](./config.js)：本地开发可以留空并使用前端 fallback，部署给测试用户时应写入 HTTPS API / LLM 地址，不用让测试用户手动改 Endpoint。
...
```text
quant-quant-leetcode-probabilty-tex/
  README.md
  BETA_LAUNCH.md
  index.html
  app.js
...
注意：`quantguide/` 是本地授权账户导出的私有题源包。内测前必须确认你有权限向测试用户分发；如果没有，请在 `data/question-banks/catalog-manifest.json` 中给 `quantguide` 加上 `"disabled": true` 后运行 `node scripts/build-problem-catalog.mjs`。
...
- 邀请制内测仍需要 HTTPS、rate limit、邮箱验证/找回方案和更严格的运维监控。
