...
- `beta.quantgym.app` 是当前内测入口；裸域 `quantgym.app` 和 `www.quantgym.app` 已通过 `quantgym-apex-redirect` Cloudflare Worker 返回 302 到 beta 入口，并由 `npm run check:apex-www-domain -- --require-clear` 签收。
