...
const catalogPath = path.join(projectRoot, "data", "question-banks", "linalg-primer", "problems.json");
const bookDir = path.join(bookRoot, "纯textbook", "金融工程线性代数入门 A Linear Algebra Primer for Financial Engineering");
const englishPath = path.join(bookDir, "quant_linalg_book.tex");
const chinesePath = path.join(bookDir, "quant_linalg_book_zh.tex");
...
function rebuildSource(filePath, language) {
  const title = language === "zh" ? "金融工程线性代数入门" : "A Linear Algebra Primer for Financial Engineering";
  const subtitle = language === "zh" ? "Reviewed QuantGym concept cards" : "Reviewed QuantGym concept cards";
