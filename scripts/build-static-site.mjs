...
function copyRuntimeStaticFiles(distDir) {
  ...
  for (const fileName of fs.readdirSync(dataDir)) {
    if (fileName.endsWith(".js") && fileName !== "problem-catalog.js") {
      fs.copyFileSync(path.join(dataDir, fileName), path.join(distDataDir, fileName));
    }
  }
  writeBrowserSafeProblemCatalog(distDir);
...
function writeBrowserSafeProblemCatalog(distDir) {
  ...
  if (/^https:\/\//i.test(scriptPath)) {
    if (source) {
      throw new Error("QUANTGYM_WEB_PROBLEM_CATALOG_SOURCE cannot be used with an HTTPS problem catalog script URL.");
    }
    return;
  }
  if (!scriptPath.startsWith("/data/") || !scriptPath.endsWith(".js") || scriptPath.includes("..")) {
    throw new Error("QUANTGYM_WEB_PROBLEM_CATALOG_SCRIPT must be a site-relative /data/*.js path or an HTTPS URL.");
  }
  ...
  if (source) {
    ...
    const problems = readProblemCatalogScript(sourcePath);
    assertExplicitlyPublicProblems(problems, `Configured browser catalog ${source}`);
    writePublicCatalog(destination, problems);
    return;
  }

  const compiledPath = path.join(projectRoot, "data", "problem-catalog.json");
  const compiled = JSON.parse(fs.readFileSync(compiledPath, "utf8"));
  if (!Array.isArray(compiled?.problems)) {
    throw new Error("data/problem-catalog.json must contain a problems array.");
  }
  const publicProblems = selectExplicitlyPublicProblems(compiled.problems);
  writePublicCatalog(destination, publicProblems);
}

function writePublicCatalog(destination, publicProblems) {
  fs.writeFileSync(destination, [
    "// Generated by scripts/build-static-site.mjs; only explicitly public problems may be shipped to browsers.",
    `window.quantProblemCatalog = ${JSON.stringify(publicProblems, null, 2)};`,
    ""
  ].join("\n"));
}

function readProblemCatalogScript(sourcePath) {
  const sandbox = { window: {} };
  vm.runInNewContext(fs.readFileSync(sourcePath, "utf8"), sandbox, {
    filename: sourcePath,
    timeout: 5000
  });
  if (!Array.isArray(sandbox.window.quantProblemCatalog)) {
    throw new Error(`Configured browser catalog must set window.quantProblemCatalog to an array: ${sourcePath}`);
  }
  return sandbox.window.quantProblemCatalog;
}
