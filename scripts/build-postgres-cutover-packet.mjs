...
function renderPostgresImportRunbook(packet) {
  return [
    ...
    "",
    "```bash",
    `python3 scripts/import-api-sqlite-export-to-postgres.py --export ${packet.migrationInputs.secureExportPath} --out ${packet.migrationInputs.postgresImportSqlPath} --replace --init-schema`,
    "```",
    "",
    ...
    "",
    "```bash",
    `python3 scripts/import-api-sqlite-export-to-postgres.py --export ${packet.migrationInputs.secureExportPath} --out ${packet.migrationInputs.postgresImportSqlPath} --replace --init-schema --execute --database-url "$QUANTGYM_POSTGRES_DATABASE_URL" --confirm-replace`,
    "```",
    "",
    "If the migration runtime has Python `psycopg` but not the `psql` CLI, use the direct Python driver:",
    "",
    "```bash",
    `python3 scripts/import-api-sqlite-export-to-postgres.py --export ${packet.migrationInputs.secureExportPath} --out ${packet.migrationInputs.postgresImportSqlPath} --replace --init-schema --execute --execute-driver psycopg --database-url "$QUANTGYM_POSTGRES_DATABASE_URL" --confirm-replace`,
    "```",
    "",
    "Use `--init-schema` for a new managed Postgres target. Use `--replace --confirm-replace` only for the approved cutover target. Do not paste or commit the managed database URL.",
    "",
    "## Post-Import",
    "",
