...
test("different active trials reject the merge without changing either record or backup", () => {
  const now = Date.parse("2026-09-09T12:00:00Z");
  const snapshots = ['device-a', 'device-b'].map((id, index) => {
    let active = createTrial({ durationSeconds: 10 }, { id, now: now + index, rng: () => 0 });
    active = transitionTrial(active, { type: 'input', value: String(active.currentQuestion.answer) }, now + 1000, () => 0);
    active = transitionTrial(active, { type: 'submit', value: '999' }, now + 2000, () => 0);
    return { ...createPersonalState(), activeTrial: active };
  });
  const before = structuredClone(snapshots);
  for (const [a, b] of [snapshots, [...snapshots].reverse()]) assert.throws(() => mergePersonalData(a, b), { code: 'active_training_conflict' });
  assert.deepEqual(snapshots, before);
  const storage = memoryStorage();
  const store = createPersonalStore({ ownerId: 'same-owner', storage });
  store.update(() => snapshots[0]);
  const backup = store.exportBackup();
  const other = createPersonalStore({ ownerId: 'same-owner', storage: memoryStorage() });
  other.update(() => snapshots[1]);
  assert.throws(() => store.restoreBackup(other.exportBackup()), { code: 'active_training_conflict' });
  assert.deepEqual(JSON.parse(store.exportBackup()).data, JSON.parse(backup).data);
  assert.deepEqual(createPersonalStore({ ownerId: 'same-owner', storage }).getSnapshot().data, snapshots[0]);
  const completed = snapshots.map(state => persistTrialTransition(state, transitionTrial(state.activeTrial, { type: 'tick' }, now + 11000)));
  const merged = mergePersonalData(...completed);
  assert.equal(merged.activeTrial, null);
  assert.equal(merged.trials.length, 2);
  assert.ok(merged.trials.every(trial => trial.correct === 1 && trial.questions.at(-1).submittedAnswer === '999' && trial.questions.at(-1).mistakes.length === 1));
  assert.equal(merged.activities.reduce((sum, activity) => sum + activity.count, 0), 2);
});

test("a stale completion cannot replace a higher-progress active snapshot; finishing locally resolves it", () => {
  const now = Date.parse("2026-09-09T12:00:00Z");
  const original = createTrial({ durationSeconds: 10 }, { id: 'foreground', now, rng: () => 0 });
  const answered = transitionTrial(original, { type: 'input', value: String(original.currentQuestion.answer) }, now + 1000, () => 0);
  const local = { ...createPersonalState(), activeTrial: answered };
  const stale = persistTrialTransition(createPersonalState(), transitionTrial(original, { type: 'tick' }, now + 10000));
  for (const [a, b] of [[local, stale], [stale, local]]) assert.throws(() => mergePersonalData(a, b), { code: 'active_training_conflict' });
  assert.equal(local.activeTrial.correct, 1);
  const finished = persistTrialTransition(local, transitionTrial(answered, { type: 'tick' }, now + 10000));
  const merged = mergePersonalData(finished, stale);
  assert.equal(merged.trials.length, 1);
  assert.equal(merged.trials[0].correct, 1);
  assert.deepEqual(merged.trials[0].questions, finished.trials[0].questions);
  assert.equal(merged.activities[0].count, 1);
});

test("stale zero-score completion also cannot discard a current answer or wrong submission", () => {
  const now = Date.parse("2026-09-09T12:00:00Z");
  const original = createTrial({ durationSeconds: 10 }, { id: 'draft', now, rng: () => 0 });
  const draft = transitionTrial(original, { type: 'submit', value: '999' }, now + 1000);
  const local = { ...createPersonalState(), activeTrial: draft };
  const stale = persistTrialTransition(createPersonalState(), transitionTrial(original, { type: 'tick' }, now + 10000));
  assert.throws(() => mergePersonalData(local, stale), { code: 'active_training_conflict' });
  const finished = persistTrialTransition(local, transitionTrial(draft, { type: 'tick' }, now + 10000));
  assert.doesNotThrow(() => mergePersonalData(local, finished));
  for (const [a, b] of [[finished, stale], [stale, finished]]) {
    const merged = mergePersonalData(a, b);
    assert.equal(merged.trials[0].questions[0].submittedAnswer, '999');
    assert.equal(merged.trials[0].questions[0].mistakes.length, 1);
  }
  assert.equal(local.activeTrial.currentAnswer, '999');
  assert.equal(local.activeTrial.currentQuestion.mistakes.length, 1);
});
