...
function ensurePrompts(problems, book, inputPath) {
  problems.forEach((problem) => {
    ...
    if (prompt) return;
    const fallback = `该条目来自《${book.name}》，但源文件未保留可显示的题面。请对照源文件补全：${inputPath}`;
...
function writeBook(book, inputPath, problems) {
  ...
  fs.writeFileSync(path.join(sourceDir, "problems.js"), [
...
function upsertManifestSource(book, inputPath, problemCount) {
  ...
  const nextSource = {
    ...
    name: book.name,
    type: "latex",
    sourceTexPath: book.disabled ? "" : inputPath,
    problemFile: `${book.slug}/problems.json`,
    browserFile: `${book.slug}/problems.js`,
