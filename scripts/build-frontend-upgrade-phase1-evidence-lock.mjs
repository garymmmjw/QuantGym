...
} from "./lib/frontend-upgrade-phase2-evidence-lock.mjs";
...
const outputPath = path.resolve(root, PHASE1_EVIDENCE_LOCK_PATH);
...
if (printOnly) {
  ...
  if (checkedIn !== serialized) {
    console.error("FAIL: Phase 1 evidence lock differs from accepted tracked Git objects");
    process.exitCode = 1;
  } else {
    console.log(`Phase 1 evidence lock current: ${lock.entryCount} tracked files.`);
  }
} else {
  ...
  console.log(`Wrote Phase 1 evidence lock for ${lock.entryCount} tracked files.`);
}
